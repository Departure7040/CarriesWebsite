...
|---|---|---|
| **Entity consistency (NAP + credentials)** | **Poor, with one corroborating signal** | 3 distinct phone numbers (337-258-5379 primary; (337) 341-8976 Homes.com; (337) 522-7554 LoopNet), 5 distinct addresses across sources, 3 distinct emails (personal Gmail, exprealty.com domain, "Southern Collective Group" Gmail), name presented as "Carrie Billeaud," "Carrie Billeaud Team," "Carrie Billeaud-Realtor EXP Realty," and now also "Carrie Billeaud Realtor \| Acadiana & Surrounding Area \| eXp Realty" on GBP (itself a policy-risk variant, see below) depending on platform, ICON Agent claim varies 2x (Zillow, Nextdoor) vs. 3x (Facebook). The GBP's phone ((337) 258-5379) and address (3 Flagg Place, Lafayette) corroborate the primary phone and the Nextdoor address variant respectively — a positive data point, but it doesn't resolve the remaining conflicts. All [verified] in `data/nap_consistency_matrix.csv` and `01_public_presence_inventory.md`. |
...
| **Authority signals (third-party)** | **Strong on Google/Zillow, thin elsewhere** | Real strengths: **Google Business Profile — 185 reviews, 5.0★** [verified, corrected 2026-07-08 — see `01_public_presence_inventory.md` Errata; this is the single strongest authority signal found in this audit]; Zillow individual profile — 29 reviews, 5.0★, strong bio [verified]; Facebook page — 5,550 likes, 142 talking-about [verified via search snippet, page itself blocked from full audit]; strong, detailed bios on Homes.com, Realtor.com, Nextdoor, LoopNet [verified]. Weaknesses: the GBP's keyword/area-decorated business name risks a Google policy action and may be suppressing how confidently AI systems and Google's own matching associate searches with this entity [verified observation / best-practice risk assessment]; low review counts on Realtor.com (2) and none on Homes.com/LoopNet despite strong bio content there; no evidence yet of local press or association features beyond a stated RAA membership [unverified]. |
| **Contactability clarity** | **Confused, with corroboration** | A visitor or an AI system trying to answer "how do I reach Carrie Billeaud" still gets three different phone numbers and three different email addresses depending on which source they land on, with no canonical designation anywhere. [verified] The GBP independently corroborates the primary phone (337-258-5379) and one address variant (3 Flagg Place), which strengthens the case for those specific values if/when Carrie makes the canonical NAP decision, but does not resolve the conflict on its own. |
...
### WHY is she credible?
- Only publish metrics that are independently verifiable or client-confirmed. The strongest verified credibility signal today is the **Google Business Profile: 185 reviews / 5.0★** [verified, corrected 2026-07-08], with Zillow's 29 reviews / 5.0★ [verified] as a strong secondary signal — these should be the anchor proof points cited everywhere, including on her own site, which currently displays zero reviews.
...
- Pick **one** canonical phone, **one** canonical email, and **one** canonical business address (this requires an explicit [client-confirm] decision from Carrie — the audit found 3 phones / 5 addresses / 3 emails and cannot pick one on her behalf). Once decided:
  - Update every owned and controllable third-party profile to match (eXp site, Zillow individual + team, Realtor.com, Homes.com, Nextdoor, LoopNet, Facebook, LinkedIn, Instagram, and the existing Google Business Profile once manager access is obtained).
...
2. **Add JSON-LD schema.** At minimum: `RealEstateAgent`/`Person` schema on `/agents.php` with `name`, `telephone`, `email`, `areaServed`, `knowsAbout` (specialties), and a `sameAs` array pointing to every verified profile (Zillow, Realtor.com, Homes.com, Nextdoor, LoopNet, LinkedIn, Facebook, Instagram, and the Google Business Profile — https://share.google/kcBY0AQWmnVjNLAy4). The `sameAs` array is the direct, standard mechanism by which search/AI systems tie together a single entity across multiple web presences — this is the technical backbone that makes "entity consistency" machine-readable rather than just human-readable. Add `BlogPosting` schema to blog content and `LocalBusiness` for the brokerage/service area once schema support exists. [verified gap, template-level]
...
| P2 | Obtain manager access to the existing Google Business Profile; fix the keyword/area-decorated name (carefully, documented); align NAP to confirmed canonical values | Entity/local | Carrie/agency | P1 address decision |
